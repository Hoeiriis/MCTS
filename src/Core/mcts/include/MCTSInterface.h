...
class MCTSInterface {
  public:
    MCTSInterface(EnvironmentBase &environment);
  ...
  protected:
    ...
    virtual std::shared_ptr<SearchNode> m_best_child(std::shared_ptr<SearchNode> node)=0;
    virtual void m_backpropagation(std::shared_ptr<SearchNode> node, int score)=0;
